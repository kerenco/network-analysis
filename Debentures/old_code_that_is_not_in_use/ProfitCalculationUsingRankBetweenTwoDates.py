...
def main():
    start_dates = {"25_01_2017"} #"12_12_2016",
    end_dates = {"11_03_2017"} #"25_01_2017",
            ...
            if (start_date is not end_date):
                for treshold in tresholds:
                    for inter in intercept:
                        ...
                        profit, num_of_comparisons = CalculateProfit(start_date, end_date, treshold, inter)
                        print("profit is :       ", profit)
                        setProfitInDB(start_date, end_date, treshold, inter, profit, num_of_comparisons)
...
def CalculateProfit(start_date,end_date, treshold,intercept):
    profit=0
    ...
    for i in companys_numbers:
        ...
        for j in companys_numbers:
            ...
            if (i_rank > j_rank):
                profit_i_j = getProfitOfInvestInHighRankAginstLowRank(i, j, start_date, end_date)
            else:
                profit_i_j = getProfitOfInvestInHighRankAginstLowRank(j, i, start_date, end_date)
        ...
        insert_company_profit_to_db(i, profit_of_company_i, start_date, end_date, treshold, intercept)

    return profit ,  num_of_comparisons


def getProfitOfInvestInHighRankAginstLowRank (high_rank_company,low_rank_company,start_date,end_date):
...
def setProfitInDB(start_date, end_date, treshold, intercept, profit, num_of_comparisons):
    ...
    insert_query = "INSERT INTO `profit_results` VALUES" \
                   ...
                   + "','comparing each company with all the rest of the omapnies');"
...
def insert_company_profit_to_db(i, profit_of_company_i, start_date, end_date, treshold, intercept):
    date_time = datetime.now()
    cnx = mysql.connector.connect(user='root', password='YOFISHELI', host='127.0.0.1', database='master_thesis')
    cursor = cnx.cursor(buffered=True)
    insert_query = "INSERT INTO `profit_company_results` VALUES" \
                   ...
                   + "','comparing each company with all the rest of the comapnies');"
    print("insert_query: ", insert_query)
    try:
        cursor.execute(insert_query)  # Execute the SQL command
        cnx.commit()  # Commit your changes in the database
