...
def main():
    graph_table="`graph12_12_2016T=0.05`"
    treshhold=0.05
    creatingGraph(graph_table,treshhold)
    print("The graph building had been finished")


def creatingGraph(graph_table,treshhold):
...
def calculateDebentureMatrix(a_debenture, b_debenture, treshhold, graph_tabel):
    ...
    if (abs(a_normal_interest - b_normal_interest) < treshhold) & (a_debenture[4] != b_debenture[4]):
